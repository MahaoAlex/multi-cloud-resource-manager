#!/usr/bin/env python3
"""
EIP scanner module for Aliyun resource optimization.
...
def run_aliyun_command(
    ...
    Args:
        service: Aliyun service (eip, etc.)
        action: Action to perform (DescribeEipAddresses, etc.)
        region: Region ID
...
def get_eips(region: str) -> List[Dict[str, Any]]:
    ...
    """
    response = run_aliyun_command('vpc', 'DescribeEipAddresses', region)

    if 'error' in response:
        return []

    # Extract EIPs from response
    eips = response.get('EipAddresses', {}).get('EipAddress', [])
...
def is_eip_unattached(eip: Dict[str, Any]) -> bool:
    ...
    # In Aliyun: 'Available' means not attached, 'InUse' means attached
    unattached_statuses = ['available', 'unattached', 'free']
...
def extract_eip_info(eip: Dict[str, Any], region: str) -> Dict[str, Any]:
    ...
    if not created_by:
        created_by = eip.get('AllocationTime', '')
