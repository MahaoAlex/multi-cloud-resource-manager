#!/usr/bin/env python3
"""
Report Generator for Aliyun Resource Manager
...
class ReportGenerator:
    ...
    def _generate_markdown(
        ...
        # Header
        lines.append("# Aliyun Resource Scan Report")
        lines.append("")
        ...
        lines.append(f"- **Public OSS Buckets:** {summary.get('public_oss_buckets', 0)}")
        ...
        if action_items:
            ...
            # Public OSS
            oss = data.get("oss_issues", [])
            if oss:
                lines.append("### Public OSS Buckets/Objects")
                lines.append("")
                for issue in oss:
        ...
        lines.append("---")
        lines.append("")
        lines.append("*Generated by Aliyun Resource Manager*")
    ...
    def _generate_action_items(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Public OSS
        for oss in data.get("oss_issues", []):
            action_items.append({
                "resource_type": "oss_bucket",
                "resource_id": oss.get("bucket_name"),
                "region": oss.get("region"),
                "severity": oss.get("risk_level", "high"),
                "issue": f"Public access: {oss.get('permission')}",
...
def main():
    ...
    test_data = {
        ...
        "summary": {
            ...
            "unused_vpcs": 2,
            "security_issues": 3,
            "public_oss_buckets": 1,
            "low_utilization_ecs": 2,
            "unattached_eips": 1,
            "naming_violations": 3
        },
        "summary_by_region": {
            "cn-hangzhou": {"vpcs": 6, "security_issues": 2, "ecs_issues": 2},
            "cn-shanghai": {"vpcs": 4, "security_issues": 1, "ecs_issues": 1}
        },
        "vpc_analysis": [
            {
                "vpc_id": "vpc-001",
