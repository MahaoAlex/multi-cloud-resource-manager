#!/usr/bin/env python3
"""
Main entry point for AWS Resource Manager Skill.
...
from ec2_monitor import monitor_ec2_instances
...
def scan_s3(regions: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(f"Starting S3 scan")

    issues = scan_s3_buckets(regions)

    logger.info(f"S3 scan complete. Found {len(issues)} issue(s)")
    return issues
...
def scan_ec2(
    ...
    logger.info(f"Starting EC2 scan for {len(regions)} region(s)")

    issues = monitor_ec2_instances(regions, cpu_threshold, check_naming)

    logger.info(f"EC2 scan complete. Found {len(issues)} issue(s)")
    return issues
...
def full_scan(
    ...
    for i, region in enumerate(regions, 1):
        ...
        try:
            ...
            # S3 scan (global but filtered by region)
            s3_issues = scan_s3_buckets([region])
            all_results["s3_issues"].extend(s3_issues)

            # EC2 scan
            ec2_issues = monitor_ec2_instances([region])
            all_results["ec2_issues"].extend(ec2_issues)

            # Naming violations from EC2 issues
            for ec2 in ec2_issues:
                for issue in ec2.get("issues", []):
                    if issue.get("type") == "naming_violation":
                        all_results["naming_violations"].append({
                            ...
                            "resource_name": ec2.get("instance_name"),
                            "region": region
                        })
            ...
            all_results["summary_by_region"][region] = {
                ...
                "s3_issues": len(s3_issues),
                "ec2_issues": len(ec2_issues),
    ...
    all_results["summary"] = {
        ...
        "public_s3_buckets": len(set(b.get("bucket_name") for b in all_results["s3_issues"])),
        "low_utilization_ec2": len([
            e for e in all_results["ec2_issues"]
...
def main():
    ...
    parser.add_argument("--scan", choices=["vpc", "security", "s3", "ec2", "eip", "full"], default="full")
